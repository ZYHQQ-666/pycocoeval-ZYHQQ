...
def _extract_category_meta(dataset):
    ...
    return cat_ids, class_names, id_to_idx
...
def _build_overall_metric_row(p, r, f1, ap):
    if len(p) == 0:
        return ['all', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A']
    map75_col = 5 if ap.shape[1] > 5 else ap.shape[1] - 1
    return [
        'all',
        _format_metric(float(np.mean(p))),
        _format_metric(float(np.mean(r))),
        _format_metric(float(np.mean(f1))),
        _format_metric(float(np.mean(ap[:, 0]))),
        _format_metric(float(np.mean(ap[:, map75_col]))),
        _format_metric(float(np.mean(ap))),
    ]


def _build_class_metric_rows(class_names, ap_class, p, r, f1, ap):
    metrics_by_class = {}
    ap_class = np.asarray(ap_class, dtype=int) if len(ap_class) else np.array([], dtype=int)
    for i, class_idx in enumerate(ap_class.tolist()):
        map75_col = 5 if ap.shape[1] > 5 else ap.shape[1] - 1
        metrics_by_class[int(class_idx)] = {
            ...
            'f1': float(f1[i]),
            'map50': float(ap[i, 0]),
            'map75': float(ap[i, map75_col]),
            'map5095': float(np.mean(ap[i])),
        }

    rows = []
    for class_idx, class_name in enumerate(class_names):
        metric = metrics_by_class.get(class_idx)
        if metric is None:
            rows.append([class_name, 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A'])
            continue
        rows.append([
            class_name,
            _format_metric(metric['precision']),
            _format_metric(metric['recall']),
            _format_metric(metric['f1']),
            _format_metric(metric['map50']),
            _format_metric(metric['map75']),
            _format_metric(metric['map5095']),
        ])
    return rows
...
def _finalize_stats(stats):
    if len(stats) == 0:
        return None
    return [torch.cat(x, 0).cpu().numpy() for x in zip(*stats)]
...
def compute_ap(recall, precision):
    ...
    if method == 'interp':
        ...
        ap = np.trapezoid(np.interp(x, mrec, mpre), x) if hasattr(np, 'trapezoid') else np.trapz(np.interp(x, mrec, mpre), x) # integrate
    else:  # 'continuous'
...
class ConfusionMatrix:
    ...
    def matrix_to_metrics(self):
        ...
        # fp = self.matrix[:, :-1].sum(0) - tp  # 按预测列统计FP
        # fn = self.matrix[:-1, :].sum(1) - tp  # 按真实行统计FN
    ...
    def print(self):
        """打印混淆矩阵"""
        self.logger.info(RED + "------------------------ Confusion Matrix ------------------------" + RESET)
        for i in range(self.nc + 1):
            self.logger.info(' '.join(map(str, self.matrix[i])))
    ...
    def plot(self, save_dir='', names=None):
        ...
        try:
            ...
            plt.close()
            self.logger.info(f"Confusion matrix saved to {save_dir / 'confusion_matrix.png'}")
...
def get_yolo_det_metrice(logger, coco_evaluator, coco_pred_json, save_dir=None):
    ...
    _, class_names, id_to_idx = _extract_category_meta(cocoGt.dataset)

    image_ids = sorted(list(cocoGt.imgs.keys()))
    image_id_dict = {i: {'pred': [], 'label': []} for i in image_ids}

    for key in cocoGt.anns:
        data = cocoGt.anns[key]
        class_idx = id_to_idx.get(int(data['category_id']))
        if class_idx is None:
            continue
        x_min, y_min, w, h = data['bbox'][0], data['bbox'][1], data['bbox'][2], data['bbox'][3]
        x_max, y_max = x_min + w, y_min + h

        image_id_dict.setdefault(data['image_id'], {'pred': [], 'label': []})
        image_id_dict[data['image_id']]['label'].append(np.array([class_idx, x_min, y_min, x_max, y_max]))
    ...
    for data in coco_pred_json:
        ...
        image_id_dict[data['image_id']]['pred'].append(np.array([x_min, y_min, x_max, y_max, float(score), class_idx]))
    
    confusion_matrix = ConfusionMatrix(nc=len(class_names), conf=0.25, iou_thres=0.5)
    confusion_matrix.set_logger(logger)
    ...
    finalized_stats = _finalize_stats(stats)
    if finalized_stats is None or finalized_stats[3].size == 0:
        p = np.array([])
        r = np.array([])
        f1 = np.array([])
        ap = np.zeros((0, 10))
        ap_class = np.array([], dtype=int)
    else:
        tp, fp, p, r, f1, ap, ap_class = ap_per_class(*finalized_stats)

    if save_dir is not None:
        confusion_matrix.plot(save_dir=save_dir, names=class_names + ['background'])

    table = PrettyTable()
    table.title = f"YOLO Metrics(bbox)"
    table.field_names = ["Classes", 'Precision', 'Recall', 'F1-Score', 'mAP50', 'mAP75', 'mAP50-95']
    table.add_row(_build_overall_metric_row(p, r, f1, ap))
    for row in _build_class_metric_rows(class_names, ap_class, p, r, f1, ap):
        table.add_row(row)
    print(table)

def get_yolo_seg_metrice(logger, coco_evaluator, coco_pred_json, save_vis=False, vis_dir='./yolo_mask_visualization'):
    ...
    image_ids = sorted(list(cocoGt.imgs.keys()))
    image_id_dict = {i: {'pred_mask': [], 'pred_cls': [], 'pred_score': [], 'label_mask': [], 'label_cls': []} for i in image_ids}
    ...
    for key in cocoGt.anns:
        ...
        image_id_dict.setdefault(data['image_id'], {'pred_mask': [], 'pred_cls': [], 'pred_score': [], 'label_mask': [], 'label_cls': []})
        image_id_dict[data['image_id']]['label_cls'].append(class_idx)
    ...
    for data in coco_pred_json:
        ...
        image_id_dict[data['image_id']]['pred_cls'].append(class_idx)
    ...
    for idx, image_id in enumerate(tqdm.tqdm(list(image_id_dict.keys()), desc="Cal YOLO(segm) mAP...")):
        label_cls = np.array(image_id_dict[image_id]['label_cls'])
