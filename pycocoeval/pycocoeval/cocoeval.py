...
class COCOeval:
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType='segm', using_tiny_metrice=False, using_vtiny_metrice=False, max_dets=100):
        '''
        ...
        self.params = Params(iouType=iouType, using_tiny_metrice=using_tiny_metrice, using_vtiny_metrice=using_vtiny_metrice, max_dets=max_dets) # parameters
        self._paramsEval = {}               # parameters for evaluation
        self.stats = []                     # result summarization
        self.ious = {}                      # ious between all gts and dts
        self.using_tiny_metrice = using_tiny_metrice
        self.using_vtiny_metrice = using_vtiny_metrice
    ...
    def evaluate_(self):
        ...
        return np.asarray(self.evalImgs).reshape(len(catIds), len(p.areaRng), len(p.imgIds))
    ...
    def summarize(self):
        ...
        def _summarizeDets_Tiny():
            ...
            stats[10] = _summarize(0, areaRng='tiny', maxDets=self.params.maxDets[2])
            ...
            return stats
        def _summarizeDets_VeryTiny():
            stats = np.zeros((16,))
            stats[0] = _summarize(1, maxDets=self.params.maxDets[2])
            stats[1] = _summarize(1, iouThr=.5, maxDets=self.params.maxDets[2])
            stats[2] = _summarize(1, iouThr=.75, maxDets=self.params.maxDets[2])
            stats[3] = _summarize(1, areaRng='vtiny', maxDets=self.params.maxDets[2])
            stats[4] = _summarize(1, areaRng='tiny', maxDets=self.params.maxDets[2])
            ...
            stats[11] = _summarize(0, areaRng='vtiny', maxDets=self.params.maxDets[2])
        ...
        if iouType == 'segm' or iouType == 'bbox':
            if self.using_vtiny_metrice:
                summarize = _summarizeDets_VeryTiny
            elif self.using_tiny_metrice:
    ...
    def merge(self, all_img_ids, all_eval_imgs):
        merged_img_ids = []
    ...
    def summarize_per_class(self):
        ...
        for idx, cat_id in enumerate(cat_ids):
            ...
            for area in ([1, 2, 3, 4, 5] if self.using_vtiny_metrice else [1, 2, 3, 4] if self.using_tiny_metrice else [1, 2, 3]):
...
class Params:
    ...
    def setDetParams(self):
        ...
        self.maxDets = [1, 10, self.max_dets]
        if self.using_vtiny_metrice:
            self.areaRng = [[0 ** 2, 1e5 ** 2], [0 ** 2, 8 ** 2], [8 ** 2, 16 ** 2], [16 ** 2, 32 ** 2], [32 ** 2, 96 ** 2], [96 ** 2, 1e5 ** 2]]
    ...
    def __init__(self, iouType='segm', using_tiny_metrice=False, using_vtiny_metrice=False, max_dets=100):
